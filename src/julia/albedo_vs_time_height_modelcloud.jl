...
# sounding data
sondepath = joinpath(homedir(),"Data/ATOMIC/radiosonde/level2")
ncfile = filter(x-> startswith(x,"EUREC4A_RonBrown") && endswith(x,"Vaisala-RS_L2_v3.0.0.nc"), readdir(sondepath))[1]
sondtime, sondz, ta, q, p = NCDataset( joinpath(sondepath, ncfile) ) do ds
    (   ds[:launch_time][:],
...
tmean(x) = sum(f0, x, dims=2) ./ sum(good, x, dims=2)
qm = tmean(q)
qs = tmean(qsat.( p, ta.-273.15 ))
...
qcb = qs[icb] # 0.01568
dz = 10.0 # m
qtilde = TradeCuModel.q_tilde(qm, qs, x)
qt = TradeCuModel.q_total(dz*ae, x, qs, qm; i0=icb, qt0=qcb)
ql = max.(0, qt.-qs)
...
plot(100*cumsum(reverse(reflec_profile_mean    .+ reflec_profile_err)), reverse(height_bins)/1e3, linewidth=0.4, color="tab:orange")
plot(0.5*100*cumsum(reverse(cloud_profile_mean .+ cloud_profile_err)), reverse(height_bins)/1e3, linewidth=0.4, color="k")
xlabel("cumulative cloud fraction (%)")
...
subplot(1,3,3)
[ plot(1e3*q[:,i], z/1e3, color="tab:gray", linestyle="none", marker=".", markersize=0.03, linewidth=0.07) for i in axes(q,2) ]
...
for ae = aeplot
    ...
    plot(1e3*max.(0,qt_.-qs), z/1e3, color="tab:green", linewidth=1.2)
end
# plot control case
# plot(1e3*qt, z/1e3, label="α+ϵ=$(1e3*ae) km\$^{-1}\$", color="tab:blue",linewidth=1.2)
# plot(1e3*ql, z/1e3, color="tab:green", linewidth=1.2)
ylim([0, 4])
xlim([-0.1, 21])
# ylabel("height (km)")
title("  radiosondes and cloud model")#\nfor different entrainment\nand autoconversion sinks")
# legend(frameon=false)
xlabel("specific humidity (g/kg)")
text( 1, 0.8, "cloud liquid \$q_l\$")
text(15.4, 0.8, "total \$q_c\$")
...
[ savefig(joinpath("shcu_cloud_albedo_refl_cloudmodel.$f")) for f in ["png", "pdf", "svg", "eps" ] ]
